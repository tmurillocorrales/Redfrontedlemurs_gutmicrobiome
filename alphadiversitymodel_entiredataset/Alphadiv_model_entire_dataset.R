...
xx.fe.re = fe.re.tab(
  ...
  re = "(1|individual) + (1|SampleID)", #include here the random effects
...
xx=lm(alpha.div ~ sex + group + z.log.age_months + ffr.prop + fle.prop + rain + ffl.prop + z.soc.int + z.richness.para,
      data=t.data)
...
full.wc = lmer(tr.alpha.div ~ sex + group + z.log.age_months + z.soc.int + ffr.prop + fle.prop + rain + ffl.prop + z.richness.para+
                 (1 + z.log.age_months + z.soc.int + ffr.prop + fle.prop + rain + ffl.prop + z.richness.para|individual), REML=F,
...
full.woc = lmer(tr.alpha.div ~ sex + group + z.log.age_months + z.soc.int + ffr.prop + fle.prop + rain + ffl.prop + z.richness.para+
                  (1 + z.log.age_months + z.soc.int + ffr.prop + fle.prop + rain + ffl.prop + z.richness.para||individual), REML=F,
                data=t.data, control = contr)

#Compare log likelihoods of both models to determine if correlations should be kept.
logLik(full.wc)# -2542.686 (df=49)
logLik(full.woc) #-2551.398 (df=21)
...
vif(full.wc)
vif(full.wc)[,3]^2
#No collinearity as the highest value was 1.433405 for parasite richness.
...
null = lmer(tr.alpha.div ~ ffr.prop + fle.prop + rain + ffl.prop +
              (1 + z.log.age_months + z.soc.int + ffr.prop + fle.prop + rain + ffl.prop + z.richness.para|individual), REML=F,
            data=t.data, control = contr)
...
full.wc.reml = lmer(tr.alpha.div ~ sex + group + z.log.age_months + z.soc.int + ffr.prop + fle.prop + rain + ffl.prop + z.richness.para+
                      (1 + z.log.age_months + z.soc.int + ffr.prop + fle.prop + rain + ffl.prop + z.richness.para|individual), REML=F,
                    data=t.data, control = contr.reml)

#Estimates model calculated with lmer
...
#(Intercept)        26.017      1.760  32.650  14.783    0.000
#sexmale             1.184      0.926  98.538   1.279    0.204
#groupB             -0.650      1.861  32.334  -0.349    0.729
#groupF             -5.358      1.960  46.604  -2.735    0.009*
#groupJ             -2.122      1.809  32.484  -1.173    0.249
#z.log.age_months   -0.055      0.466 160.782  -0.117    0.907
#z.soc.int           0.869      0.487  11.429   1.786    0.101
#ffr.prop           -5.335      4.528  29.254  -1.178    0.248
#fle.prop           67.495     12.385  65.573   5.450    0.000*
#rain               -1.760      0.982   6.645  -1.793    0.118
#ffl.prop           -4.492     16.226  11.323  -0.277    0.787
#z.richness.para    -1.017      0.647  23.165  -1.573    0.129
